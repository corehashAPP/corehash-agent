...
= 0.7.6 =
* The addresses Corehash connects from are fetched from Corehash instead of fixed in the plugin, so a change of hosting IP no longer cuts a site off
* A rejected address now returns its own error, so the dashboard can tell an address problem from a token problem

= 0.7.5 =
* Moved the hardening file into a subfolder so WordPress no longer lists it as a separate plugin
...
* New patterns can be rolled out without updating the plugin

= 0.7.2 =
* The hardening file is written without a literal PHP opening tag in the source, so host scanners stop reading the agent as a dropper

= 0.7.1 =
* The malware signatures are no longer stored as plain text, so host-side scanners stop mistaking the agent itself for a webshell and neutralising it
